# InputHintSetting バグ修正準備計画

作業日: 2026-06-26

## 1. 修正対象バグ

- BUG-016: No.1 2026-24-06 BUG-016 空のサブテーブルで存在チェックがTypeErrorになる
- BUG-006: No.2 2026-24-06 BUG-006 mobile.jsのサブテーブル行追加イベントがdesktopイベント名になっている

## 2. 優先順位

1. BUG-016: 重大度 High、確度 高確度
2. BUG-006: 重大度 Medium、確度 高確度
...
- BUG-016: existenceCheck（47行付近）
- BUG-006: tableChangeEventCreate（70行付近）

## 5. 修正方針

- BUG-016: 空行時はDOM/フィールド定義ベースで判定する。mobile.jsも同様確認。
- BUG-006: `mobile.app.record.*` のテーブル変更イベントへ置換する。

## 6. 影響範囲

- BUG-016: 高 - 空のサブテーブルで入力ヒント設定処理が落ちる可能性がある。
- BUG-006: 中 - モバイルでサブテーブル行追加後、追加行に入力ヒントが付与されない。
