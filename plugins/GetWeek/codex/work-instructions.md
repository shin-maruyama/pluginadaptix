...
```text
plugins/GetWeek/GetWeek
```

## 難読化版場所

```text
plugins/GetWeek/GetWeeke
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/GetWeek/GetWeek`
- 難読化版: `plugins/GetWeek/GetWeeke`
...
1. `plugins/GetWeek/specification.md` を確認する。
2. `plugins/GetWeek/codex/troubleshooting.md`、`plugins/GetWeek/codex/handover-YYYY-MM-DD.md`、`plugins/GetWeek/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/GetWeek/GetWeek` であること。
- 難読化済みファイル `plugins/GetWeek/GetWeeke` を直接編集していないこと。
- `plugins/GetWeek/GetWeek/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/GetWeek/GetWeek` から実行します。
- 難読化済みファイルは `plugins/GetWeek/GetWeeke` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/GetWeek/GetWeek` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/GetWeek/GetWeeke` の `contents` 配下を基準に作成します。
...
```text
plugins/GetWeek/codex/handover-YYYY-MM-DD.md
plugins/GetWeek/codex/next-tasks.md
```
...
```text
GetWeek-v<version>.zip
```

難読化版ZIP:

```text
GetWeek-ev<version>.zip
```
