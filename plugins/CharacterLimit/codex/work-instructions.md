...
```text
plugins/CharacterLimit/CharacterLimit
```

## 難読化版場所

```text
plugins/CharacterLimit/CharacterLimite
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/CharacterLimit/CharacterLimit`
- 難読化版: `plugins/CharacterLimit/CharacterLimite`
...
1. `plugins/CharacterLimit/specification.md` を確認する。
2. `plugins/CharacterLimit/codex/troubleshooting.md`、`plugins/CharacterLimit/codex/handover-YYYY-MM-DD.md`、`plugins/CharacterLimit/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/CharacterLimit/CharacterLimit` であること。
- 難読化済みファイル `plugins/CharacterLimit/CharacterLimite` を直接編集していないこと。
- `plugins/CharacterLimit/CharacterLimit/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/CharacterLimit/CharacterLimit` から実行します。
- 難読化済みファイルは `plugins/CharacterLimit/CharacterLimite` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/CharacterLimit/CharacterLimit` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/CharacterLimit/CharacterLimite` の `contents` 配下を基準に作成します。
...
```text
plugins/CharacterLimit/codex/handover-YYYY-MM-DD.md
plugins/CharacterLimit/codex/next-tasks.md
```
...
```text
CharacterLimit-v<version>.zip
```

難読化版ZIP:

```text
CharacterLimit-ev<version>.zip
```
...
```text
plugins/CharacterLimit/decisions/
plugins/CharacterLimit/codex/troubleshooting.md
```
