...
```text
plugins/非表示/FieldHidden
```

## 難読化版場所

```text
plugins/非表示/FieldHiddene
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/非表示/FieldHidden`
- 難読化版: `plugins/非表示/FieldHiddene`
...
1. `plugins/非表示/specification.md` を確認する。
2. `plugins/非表示/codex/troubleshooting.md`、`plugins/非表示/codex/handover-YYYY-MM-DD.md`、`plugins/非表示/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/非表示/FieldHidden` であること。
- 難読化済みファイル `plugins/非表示/FieldHiddene` を直接編集していないこと。
- `plugins/非表示/FieldHidden/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/非表示/FieldHidden` から実行します。
- 難読化済みファイルは `plugins/非表示/FieldHiddene` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/非表示/FieldHidden` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/非表示/FieldHiddene` の `contents` 配下を基準に作成します。
...
```text
plugins/非表示/codex/handover-YYYY-MM-DD.md
plugins/非表示/codex/next-tasks.md
```
