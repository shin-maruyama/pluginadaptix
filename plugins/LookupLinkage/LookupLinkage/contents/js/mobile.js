  ...
  obj.createEvents = function () {
    if (Object.keys(config).length) {
      if (config.lookupField.copyField && config.lookupField.copyField !== '') {
        obj.events.push('mobile.app.record.create.change.' + obj.getCodePart(config.lookupField.copyField));
        obj.events.push('mobile.app.record.edit.change.' + obj.getCodePart(config.lookupField.copyField));
      }
    }
  };
