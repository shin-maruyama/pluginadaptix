...
```text
plugins/RecordRockb/RecordRockb
```

## 難読化版場所

```text
plugins/RecordRockb/RecordRockbe
```
...
1. `plugins/RecordRockb/specification.md` を確認する。
2. `plugins/RecordRockb/codex/troubleshooting.md`、`plugins/RecordRockb/codex/handover-YYYY-MM-DD.md`、`plugins/RecordRockb/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/RecordRockb/RecordRockb` であること。
- 難読化済みファイル `plugins/RecordRockb/RecordRockbe` を直接編集していないこと。
- `plugins/RecordRockb/RecordRockb/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/RecordRockb/RecordRockb` から実行します。
- 難読化済みファイルは `plugins/RecordRockb/RecordRockbe` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/RecordRockb/RecordRockb` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/RecordRockb/RecordRockbe` の `contents` 配下を基準に作成します。
...
```text
plugins/RecordRockb/codex/handover-YYYY-MM-DD.md
plugins/RecordRockb/codex/next-tasks.md
```
...
```text
RecordRockb-v<version>.zip
```

難読化版ZIP:

```text
RecordRockb-ev<version>.zip
```
