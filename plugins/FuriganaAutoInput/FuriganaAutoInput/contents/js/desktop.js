...
(async function ($, PLUGIN_ID) {
  ...
  const obj = {
    ...
    events: {
      show: ['app.record.create.show', 'app.record.edit.show'],
    ...
    furiganaConversion: function (setting, record) {
      ...
      if (setting.sourceField.code.split('　').length === 2) {
        ...
        if (record[tableCode]) {
          ...
          for (const row of $(`.subtable-${subTableId} tbody tr`)) {
    ...
    existenceCheck: function (setting, { record }) {
      const sourceSplit = setting.sourceField.code.split('　');
      const sourceName = sourceSplit.length === 1 ? sourceSplit[0] : sourceSplit[1];
      const destinationSplit = setting.destinationField.code.split('　');
      ...
      if (tableCode == '') {
        const sourceField = record[sourceName];
        const destinationField = record[destinationName];
        if (!(sourceField && destinationField)) return true;
        return false;
      } else {
        const tableField = record[tableCode];
        const sourceField = tableField.value[0].value[sourceName];
        const destinationField = tableField.value[0].value[destinationName];
        if (!(sourceField && destinationField)) return true;
        return false;
      }
    },
    ...
    getFieldList: async function () {
      const fieldList = [];
      try {
        const resp = await callKintoneApi(kintone.api.url('/k/v1/app/form/layout.json', true), 'GET', { app: kintone.app.getId() });
