...
(async function ($, PLUGIN_ID) {
  ...
  for (let i = 0, length = config.settings.length; i < length; i++) {
    ...
    changeEvents.push('mobile.app.record.create.change.' + dateField); // 新規レコード作成画面で日付フィールドの値を変更時発生イベント
    changeEvents.push('mobile.app.record.edit.change.' + dateField);   // レコード編集画面で日付フィールドの値を変更時発生イベント
  ...
  const showEvents = [
    'mobile.app.record.create.show', // 新規レコード作成画面表示時発生イベント
    'mobile.app.record.edit.show', // レコード編集画面表示時発生イベント
  ];
  ...
  kintone.events.on('mobile.app.record.index.show', async function (e) {
    ...
    const headerMenuSpace = kintone.mobile.app.getHeaderSpaceElement(); // レコード一覧画面のヘッダーメニュー部分の要素取得

    const button = document.createElement('button'); // ボタンタグ作成
    button.id = 'update-button'; // ボタンのid設定
    button.innerText = '期間更新'; // ボタンの表示文字設定
    button.style = 'width: 100%; height: 40px'; // ボタンのCSS設定

    //ボタン押下時の処理
    button.onclick = () => {
      const params = {
        app: kintone.mobile.app.getId(), // アプリID
        records: [], // 更新するレコード配列
      };
