# PeriodBulkUpdate バグ修正準備計画

作業日: 2026-06-26

## 1. 修正対象バグ

- BUG-024: No.1 2026-24-06 BUG-024 旧バージョン設定移行で固定プラグインIDを参照している

## 2. 優先順位

1. BUG-024: 重大度 Medium、確度 要確認
...
- BUG-024: 無名イベントハンドラまたはトップレベル処理

## 5. 修正方針

- BUG-024: 正式な旧プラグインID管理方針を確認し、設定移行仕様として妥当か判断する。

## 6. 影響範囲

- BUG-024: 中 - 環境や再パッケージによりプラグインIDが異なる場合、旧設定移行が機能しない。
