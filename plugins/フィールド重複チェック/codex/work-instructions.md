...
```text
plugins/フィールド重複チェック/DoubleCheck
```

## 難読化版場所

```text
plugins/フィールド重複チェック/DoubleChecke
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/フィールド重複チェック/DoubleCheck`
- 難読化版: `plugins/フィールド重複チェック/DoubleChecke`
...
1. `plugins/フィールド重複チェック/specification.md` を確認する。
2. `plugins/フィールド重複チェック/codex/troubleshooting.md`、`plugins/フィールド重複チェック/codex/handover-YYYY-MM-DD.md`、`plugins/フィールド重複チェック/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/フィールド重複チェック/DoubleCheck` であること。
- 難読化済みファイル `plugins/フィールド重複チェック/DoubleChecke` を直接編集していないこと。
- `plugins/フィールド重複チェック/DoubleCheck/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/フィールド重複チェック/DoubleCheck` から実行します。
- 難読化済みファイルは `plugins/フィールド重複チェック/DoubleChecke` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/フィールド重複チェック/DoubleCheck` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/フィールド重複チェック/DoubleChecke` の `contents` 配下を基準に作成します。
...
```text
plugins/フィールド重複チェック/codex/handover-YYYY-MM-DD.md
plugins/フィールド重複チェック/codex/next-tasks.md
```
