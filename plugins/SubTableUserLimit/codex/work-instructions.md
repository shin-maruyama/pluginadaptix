...
```text
plugins/SubTableUserLimit/SubTableUserLimit
```

## 難読化版場所

```text
plugins/SubTableUserLimit/SubTableUserLimite
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/SubTableUserLimit/SubTableUserLimit`
- 難読化版: `plugins/SubTableUserLimit/SubTableUserLimite`
...
1. `plugins/SubTableUserLimit/specification.md` を確認する。
2. `plugins/SubTableUserLimit/codex/troubleshooting.md`、`plugins/SubTableUserLimit/codex/handover-YYYY-MM-DD.md`、`plugins/SubTableUserLimit/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/SubTableUserLimit/SubTableUserLimit` であること。
- 難読化済みファイル `plugins/SubTableUserLimit/SubTableUserLimite` を直接編集していないこと。
- `plugins/SubTableUserLimit/SubTableUserLimit/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/SubTableUserLimit/SubTableUserLimit` から実行します。
- 難読化済みファイルは `plugins/SubTableUserLimit/SubTableUserLimite` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/SubTableUserLimit/SubTableUserLimit` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/SubTableUserLimit/SubTableUserLimite` の `contents` 配下を基準に作成します。
...
```text
plugins/SubTableUserLimit/codex/handover-YYYY-MM-DD.md
plugins/SubTableUserLimit/codex/next-tasks.md
```
...
```text
plugins/SubTableUserLimit/decisions/
plugins/SubTableUserLimit/codex/troubleshooting.md
```
