...
```text
plugins/RecordLimit/RecordLimite
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/RecordLimit/RecordLimit`
- 難読化版: `plugins/RecordLimit/RecordLimite`
...
1. `plugins/RecordLimit/specification.md` を確認する。
2. `plugins/RecordLimit/codex/troubleshooting.md`、`plugins/RecordLimit/codex/handover-YYYY-MM-DD.md`、`plugins/RecordLimit/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/RecordLimit/RecordLimit` であること。
- 難読化済みファイル `plugins/RecordLimit/RecordLimite` を直接編集していないこと。
- `plugins/RecordLimit/RecordLimit/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/RecordLimit/RecordLimit` から実行します。
- 難読化済みファイルは `plugins/RecordLimit/RecordLimite` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/RecordLimit/RecordLimit` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/RecordLimit/RecordLimite` の `contents` 配下を基準に作成します。
...
```text
plugins/RecordLimit/codex/handover-YYYY-MM-DD.md
plugins/RecordLimit/codex/next-tasks.md
```
...
```text
RecordLimit-v<version>.zip
```

難読化版ZIP:

```text
RecordLimit-ev<version>.zip
```
