...
```text
plugins/appList/appList
```

## 難読化版場所

```text
plugins/appList/appListe
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/appList/appList`
- 難読化版: `plugins/appList/appListe`
...
1. `plugins/appList/specification.md` を確認する。
2. `plugins/appList/codex/troubleshooting.md`、`plugins/appList/codex/handover-YYYY-MM-DD.md`、`plugins/appList/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/appList/appList` であること。
- 難読化済みファイル `plugins/appList/appListe` を直接編集していないこと。
- `plugins/appList/appList/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/appList/appList` から実行します。
- 難読化済みファイルは `plugins/appList/appListe` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/appList/appList` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/appList/appListe` の `contents` 配下を基準に作成します。
...
```text
plugins/appList/codex/handover-YYYY-MM-DD.md
plugins/appList/codex/next-tasks.md
```
