...
(async function ($, PLUGIN_ID) {
  ...
  obj.eventStart = function () {
    ...
    kintone.events.on('mobile.app.record.index.show', async function () {
      ...
      for (let i = 0; i < config.settings.length; i++) {
        ...
        const destinationList = await that.getFieldList(kintone.mobile.app.getId());
    ...
    kintone.events.on(['mobile.app.record.create.show', 'mobile.app.record.edit.show'], async function (event) {
      ...
      try {
        ...
        const layout = await callKintoneApi('/k/v1/app/form/layout.json', 'GET', { app: kintone.mobile.app.getId() })
  ...
  obj.getAllApps = async function (offset = 0, limit = 100, allApps = []) {
    const params = {
      offset,
      limit,
    };
    try {
      const { apps } = await callKintoneApi(kintone.api.url('/k/v1/apps', true), 'GET', params);
      allApps = allApps.concat(apps);
      if (apps.length === limit) {
        return await getAllApps(offset + limit, limit, allApps);
      }
    } catch { }
    return allApps;
  };
