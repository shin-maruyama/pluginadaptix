...
```text
plugins/TabDisplay/TabDisplay
```

## 難読化版場所

```text
plugins/TabDisplay/TabDisplaye
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/TabDisplay/TabDisplay`
- 難読化版: `plugins/TabDisplay/TabDisplaye`
...
1. `plugins/TabDisplay/specification.md` を確認する。
2. `plugins/TabDisplay/codex/troubleshooting.md`、`plugins/TabDisplay/codex/handover-YYYY-MM-DD.md`、`plugins/TabDisplay/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/TabDisplay/TabDisplay` であること。
- 難読化済みファイル `plugins/TabDisplay/TabDisplaye` を直接編集していないこと。
- `plugins/TabDisplay/TabDisplay/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/TabDisplay/TabDisplay` から実行します。
- 難読化済みファイルは `plugins/TabDisplay/TabDisplaye` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/TabDisplay/TabDisplay` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/TabDisplay/TabDisplaye` の `contents` 配下を基準に作成します。
...
```text
plugins/TabDisplay/codex/handover-YYYY-MM-DD.md
plugins/TabDisplay/codex/next-tasks.md
```
...
```text
TabDisplay-v<version>.zip
```

難読化版ZIP:

```text
TabDisplay-ev<version>.zip
```
