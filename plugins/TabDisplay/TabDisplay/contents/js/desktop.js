...
(async function ($, PLUGIN_ID) {
  ...
  const obj = {
    ...
    events: ['app.record.create.show', 'app.record.edit.show', 'app.record.detail.show'],
    ...
    allFieldHide: function () {
      const fieldList = obj.getFieldList();
      fieldList.forEach((field) => {kintone.app.record.setFieldShown(field.code, false);
          let fieldWrap2 = null;
          if(field.type != "SUBTABLE"){
            fieldWrap2 = document.querySelector(`.field-${field.id}`);
          }else{
          ...
          if(fieldWrap2.hasAttribute('dropdownplugin')){
            ...
            hiddenFieldParse.forEach((x) => {
              x.fields.forEach((y) => {
                kintone.app.record.setFieldShown(y, false)
              })
            })
          }
          if(fieldWrap2.hasAttribute('radioButtonPlugin')){
            const hiddenField = fieldWrap2.getAttribute('radioButtonPlugin')
            const hiddenFieldParse = JSON.parse(hiddenField)
            hiddenFieldParse.forEach((x) => {
              x.fields.forEach((y) => {
                kintone.app.record.setFieldShown(y, false)
    ...
    allFieldShow: function () {
      ...
      fieldList.forEach((field) => {
        obj.mainAndRelatedFieldShow(field);
      });
    },
    ...
    firstTabField: function () {
      obj.allFieldHide();
      const fieldList = []
      //[非表示プラグインで非表示にされていないかを確認する。]
      obj.config.settings[0].fields.forEach((val) => {
        obj.mainAndRelatedFieldShow(val);
      })
    ...
    createTab: function () {
      ...
      //[スペースフィールドの要素取得]
      const space = kintone.app.record.getSpaceElement(spaceFieldId);
     ...
     //[ドロップダウン切替プラグイン、ラジオボタン切替プラグインが適用されている場合に、
    //本体及び本隊に関連付けられているフィールドだけを]
    mainAndRelatedFieldShow: function (field) {
      let fieldWrap2 = null;
      if(field.type != "SUBTABLE"){
        fieldWrap2 = document.querySelector(`.field-${field.id}`);
      }else{
      ...
      if(fieldWrap2.hasAttribute('field-hidden-by')) return;
      ...
      if(fieldWrap2.hasAttribute('dropdownplugin')){
        ...
        getAttributeTarget.fields.forEach((split) => {
          if(!obj.hasAttributefieldHidden(split)){
            kintone.app.record.setFieldShown(split, true)
          }
        })
      }
      if(fieldWrap2.hasAttribute('radioButtonPlugin')){
         ...
         const getAttributeParse = JSON.parse(getAttribute)
         const querySelector = kintone.app.record.getFieldElement(field.code)
         let option = null;
    ...
    hasAttributefieldHidden: function (field) {
  ...
  kintone.events.on('app.record.index.show', obj.checkFields);
