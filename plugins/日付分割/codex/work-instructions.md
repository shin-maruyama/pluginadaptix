...
```text
plugins/日付分割/DateSplit
```

## 難読化版場所

```text
plugins/日付分割/DateSplite
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/日付分割/DateSplit`
- 難読化版: `plugins/日付分割/DateSplite`
...
1. `plugins/日付分割/specification.md` を確認する。
2. `plugins/日付分割/codex/troubleshooting.md`、`plugins/日付分割/codex/handover-YYYY-MM-DD.md`、`plugins/日付分割/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/日付分割/DateSplit` であること。
- 難読化済みファイル `plugins/日付分割/DateSplite` を直接編集していないこと。
- `plugins/日付分割/DateSplit/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/日付分割/DateSplit` から実行します。
- 難読化済みファイルは `plugins/日付分割/DateSplite` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/日付分割/DateSplit` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/日付分割/DateSplite` の `contents` 配下を基準に作成します。
...
```text
plugins/日付分割/codex/handover-YYYY-MM-DD.md
plugins/日付分割/codex/next-tasks.md
```
...
```text
DateSplit-v<version>.zip
```

難読化版ZIP:

```text
DateSplit-ev<version>.zip
```
