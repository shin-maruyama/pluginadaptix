# DoubleCheck バグ修正準備計画

作業日: 2026-06-26

## 1. 修正対象バグ

- BUG-003: No.1 2026-24-06 BUG-003 モバイル編集保存時に自レコード除外判定が効かない

## 2. 優先順位

1. BUG-003: 重大度 High、確度 高確度
...
- BUG-003: obj.eventStart（30行付近）

## 5. 修正方針

- BUG-003: `mobile.app.record.edit.submit` を判定する。

## 6. 影響範囲

- BUG-003: 高 - モバイル編集時、自レコードが重複チェック対象から除外されず、既存値のまま保存しても重複エラーになる可能性がある。
