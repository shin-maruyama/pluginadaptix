...
(async function ($, PLUGIN_ID) {
  ...
  if (config.settings && config.settings.length) {
    for (const setting of config.settings) {
      ...
      obj.events1.type.push('app.record.edit.change.' + field);
      obj.events1.type.push('app.record.create.change.' + field);
      obj.events1.type.push('app.record.index.edit.change.' + field);
    }
  }
