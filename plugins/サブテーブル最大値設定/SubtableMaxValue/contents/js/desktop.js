...
(async function ($, PLUGIN_ID) {
   ...
   * [チェンジイベント実行処理]
   */
  tableMaxLimit.eventStart = function () {
    let self = this;
    ...
    kintone.events.on(['app.record.edit.show','app.record.create.show'], function (event) {
      ...
      butonnArrange()//レコード編集ビュー、レコード追加ビュー表示時

      const targetElement = document.querySelectorAll('.ui-sortable');
      targetElement.forEach((elm) => {
        const observer = new MutationObserver((x) => {butonnArrange()});
        observer.observe(elm, { attributes: true });
      })

      function butonnArrange(){
      //指定したテーブル分ループ
      for (let i = 0; i < config.tableSelect.length; i++) {
        ...
        const tbody = $(".subtable-row-" + targetField[0].id).find("tbody")[0];
        const rows = tbody.getElementsByClassName("add-row-image-gaia");

        if (
          rows.length + 1 > config.limitNumber[i]
        ) {
          ...
          for (let j = 0; j < rows.length; j++) {
            rows[j].style.display  = "inline-block";
          }
        }
      }
    }
    ...
    kintone.events.on(self.changeEventCreate(config), function (event) {
      ...
      let record = event.record;
      let changeField = event.type.split('.')[4];
  ...
  tableMaxLimit.changeEventCreate = function (_config) {
    ...
    for (let i = 0; i < config.tableSelect.length; i++) {
      events.push('app.record.edit.change.' + config.tableSelect[i]);
      events.push('app.record.create.change.' + config.tableSelect[i]);
    }
