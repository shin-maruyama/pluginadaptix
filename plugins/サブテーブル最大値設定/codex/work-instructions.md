...
```text
plugins/サブテーブル最大値設定/SubtableMaxValue
```

## 難読化版場所

```text
plugins/サブテーブル最大値設定/SubtableMaxValuee
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/サブテーブル最大値設定/SubtableMaxValue`
- 難読化版: `plugins/サブテーブル最大値設定/SubtableMaxValuee`
...
1. `plugins/サブテーブル最大値設定/specification.md` を確認する。
2. `plugins/サブテーブル最大値設定/codex/troubleshooting.md`、`plugins/サブテーブル最大値設定/codex/handover-YYYY-MM-DD.md`、`plugins/サブテーブル最大値設定/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/サブテーブル最大値設定/SubtableMaxValue` であること。
- 難読化済みファイル `plugins/サブテーブル最大値設定/SubtableMaxValuee` を直接編集していないこと。
- `plugins/サブテーブル最大値設定/SubtableMaxValue/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/サブテーブル最大値設定/SubtableMaxValue` から実行します。
- 難読化済みファイルは `plugins/サブテーブル最大値設定/SubtableMaxValuee` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/サブテーブル最大値設定/SubtableMaxValue` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/サブテーブル最大値設定/SubtableMaxValuee` の `contents` 配下を基準に作成します。
...
```text
plugins/サブテーブル最大値設定/codex/handover-YYYY-MM-DD.md
plugins/サブテーブル最大値設定/codex/next-tasks.md
```
