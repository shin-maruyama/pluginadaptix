  ...
  kintone.events.on(['mobile.app.record.create.show', 'mobile.app.record.edit.show'], async function (event) {
    ...
    for (let i = 0; i < config.settings.length; i++) {
      ...
      if (tableCode == '') {
        ...
        const events = [];
        events.push('mobile.app.record.edit.change.' + starttimeSelect);
        events.push('mobile.app.record.create.change.' + starttimeSelect);
        events.push('mobile.app.record.edit.change.' + endtimeSelect);
        events.push('mobile.app.record.create.change.' + endtimeSelect);
  ...
  function tableChangeEventCreate(record) {
    ...
    for (let i = 0; i < config.settings.length; i++) {
      if (config.settings[i].starttimeSelect.split('　').length === 2 && record[config.settings[i].starttimeSelect.split('　')[0]]) {
        ...
        kintone.events.on([`mobile.app.record.create.change.${tableCode}`, `mobile.app.record.edit.change.${tableCode}`], function (event) {
          if (!event.changes.row || !event.changes.row.value) return event;
          //const { record } = event;
