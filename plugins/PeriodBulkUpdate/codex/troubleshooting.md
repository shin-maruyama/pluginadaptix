...
## 2026-26-06 BUG ID: BUG-024

### 発生事象

旧バージョン設定移行で固定プラグインIDを参照している

### 原因

旧バージョン設定移行で固定プラグインIDだけを参照していた。

### 修正内容

旧設定移行時に現在のPLUGIN_IDと既知の旧PLUGIN_IDを順に確認するようにし、固定IDのみへの依存を緩和しました。
...
node --check 成功。旧設定移行の候補ID確認処理を静的確認済み。旧バージョン実データでの確認は未実施。
