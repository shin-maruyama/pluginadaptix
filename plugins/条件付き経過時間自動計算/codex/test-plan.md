# ConditionalElapsedTimeAutomaticCalculation テスト計画書
...
- 通常版ZIP名が `ConditionalElapsedTimeAutomaticCalculation-v<version>.zip` になっている。
- 難読化版ZIP名が `ConditionalElapsedTimeAutomaticCalculation-ev<version>.zip` になっている。
...
- 既存関数、既存共通処理、既存UIを優先していること。
- 不要なリファクタリング、命名変更、ファイル移動がないこと。
- `var`、グローバル変数、不要なインラインCSSが追加されていないこと。
- 固定値、フィールドコード、イベント名が必要に応じて定数化されていること。
- 同一DOM取得、同一API処理の重複がないこと。
- 1関数1責務、関数長100行以内、ネスト3階層以内を大きく逸脱していないこと。
- 不要な `console.log`、`console.error`、`console.warn` が残っていないこと。
- REST API、添付ファイル、JSON処理、外部通信に必要なエラー処理があること。
- サブテーブル、添付ファイルの存在確認があること。
- PC、モバイル、設定保存、権限、サブテーブル、添付ファイルへの影響を確認していること。
