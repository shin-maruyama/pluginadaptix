# HandwrittenSignature Codex作業指示書
...
```text
plugins/HandwrittenSignature/HandwrittenSignature
```

## 難読化版場所

```text
plugins/HandwrittenSignature/HandwrittenSignaturee
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/HandwrittenSignature/HandwrittenSignature`
- 難読化版: `plugins/HandwrittenSignature/HandwrittenSignaturee`
...
1. `plugins/HandwrittenSignature/specification.md` を確認する。
2. `plugins/HandwrittenSignature/codex/troubleshooting.md`、`plugins/HandwrittenSignature/codex/handover-YYYY-MM-DD.md`、`plugins/HandwrittenSignature/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/HandwrittenSignature/HandwrittenSignature` であること。
- 難読化済みファイル `plugins/HandwrittenSignature/HandwrittenSignaturee` を直接編集していないこと。
- `plugins/HandwrittenSignature/HandwrittenSignature/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/HandwrittenSignature/HandwrittenSignature` から実行します。
- 難読化済みファイルは `plugins/HandwrittenSignature/HandwrittenSignaturee` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/HandwrittenSignature/HandwrittenSignature` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/HandwrittenSignature/HandwrittenSignaturee` の `contents` 配下を基準に作成します。
...
```text
plugins/HandwrittenSignature/codex/handover-YYYY-MM-DD.md
plugins/HandwrittenSignature/codex/next-tasks.md
```
...
```text
HandwrittenSignature-v<version>.zip
```

難読化版ZIP:

```text
HandwrittenSignature-ev<version>.zip
```
...
```text
plugins/HandwrittenSignature/decisions/
plugins/HandwrittenSignature/codex/troubleshooting.md
```
