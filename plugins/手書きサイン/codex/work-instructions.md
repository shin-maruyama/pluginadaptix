...
```text
plugins/手書きサイン/HandwrittenSignature
```

## 難読化版場所

```text
plugins/手書きサイン/HandwrittenSignaturee
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/手書きサイン/HandwrittenSignature`
- 難読化版: `plugins/手書きサイン/HandwrittenSignaturee`
...
1. `plugins/手書きサイン/specification.md` を確認する。
2. `plugins/手書きサイン/codex/troubleshooting.md`、`plugins/手書きサイン/codex/handover-YYYY-MM-DD.md`、`plugins/手書きサイン/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/手書きサイン/HandwrittenSignature` であること。
- 難読化済みファイル `plugins/手書きサイン/HandwrittenSignaturee` を直接編集していないこと。
- `plugins/手書きサイン/HandwrittenSignature/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/手書きサイン/HandwrittenSignature` から実行します。
- 難読化済みファイルは `plugins/手書きサイン/HandwrittenSignaturee` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/手書きサイン/HandwrittenSignature` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/手書きサイン/HandwrittenSignaturee` の `contents` 配下を基準に作成します。
...
```text
plugins/手書きサイン/codex/handover-YYYY-MM-DD.md
plugins/手書きサイン/codex/next-tasks.md
```
