...
```text
plugins/アプリ一覧（カスタム版）/appList
```

## 難読化版場所

```text
plugins/アプリ一覧（カスタム版）/appListe
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/アプリ一覧（カスタム版）/appList`
- 難読化版: `plugins/アプリ一覧（カスタム版）/appListe`
...
1. `plugins/アプリ一覧（カスタム版）/specification.md` を確認する。
2. `plugins/アプリ一覧（カスタム版）/codex/troubleshooting.md`、`plugins/アプリ一覧（カスタム版）/codex/handover-YYYY-MM-DD.md`、`plugins/アプリ一覧（カスタム版）/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/アプリ一覧（カスタム版）/appList` であること。
- 難読化済みファイル `plugins/アプリ一覧（カスタム版）/appListe` を直接編集していないこと。
- `plugins/アプリ一覧（カスタム版）/appList/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/アプリ一覧（カスタム版）/appList` から実行します。
- 難読化済みファイルは `plugins/アプリ一覧（カスタム版）/appListe` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/アプリ一覧（カスタム版）/appList` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/アプリ一覧（カスタム版）/appListe` の `contents` 配下を基準に作成します。
...
```text
plugins/アプリ一覧（カスタム版）/codex/handover-YYYY-MM-DD.md
plugins/アプリ一覧（カスタム版）/codex/next-tasks.md
```
...
```text
appList-v<version>.zip
```

難読化版ZIP:

```text
appList-ev<version>.zip
```
