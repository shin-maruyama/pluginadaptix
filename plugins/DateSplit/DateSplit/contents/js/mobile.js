...
(async function ($, PLUGIN_ID) {
  ...
  total.eventCreate = function () {
    ...
    events.push('mobile.app.record.edit.show');
    events.push('mobile.app.record.create.show');

    for (let i = 0; i < config.settings.length; i++) {
      if ((config.settings[i].dateSelect).indexOf('　') == -1) {
        events.push('mobile.app.record.edit.change.' + config.settings[i].dateSelect);
        events.push('mobile.app.record.create.change.' + config.settings[i].dateSelect);
      } else {
        var str = config.settings[i].dateSelect.split('　')[1];
        events.push('mobile.app.record.edit.change.' + str);
        events.push('mobile.app.record.create.change.' + str);
      }
    }
