...
```text
plugins/DateSplit/DateSplit
```

## 難読化版場所

```text
plugins/DateSplit/DateSplite
```
...
1. `plugins/DateSplit/specification.md` を確認する。
2. `plugins/DateSplit/codex/troubleshooting.md`、`plugins/DateSplit/codex/handover-YYYY-MM-DD.md`、`plugins/DateSplit/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/DateSplit/DateSplit` であること。
- 難読化済みファイル `plugins/DateSplit/DateSplite` を直接編集していないこと。
- `plugins/DateSplit/DateSplit/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/DateSplit/DateSplit` から実行します。
- 難読化済みファイルは `plugins/DateSplit/DateSplite` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/DateSplit/DateSplit` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/DateSplit/DateSplite` の `contents` 配下を基準に作成します。
...
```text
plugins/DateSplit/codex/handover-YYYY-MM-DD.md
plugins/DateSplit/codex/next-tasks.md
```
