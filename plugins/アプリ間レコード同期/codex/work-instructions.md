...
```text
plugins/アプリ間レコード同期/CrossAppRecordSynce
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/アプリ間レコード同期/CrossAppRecordSync`
- 難読化版: `plugins/アプリ間レコード同期/CrossAppRecordSynce`
...
1. `plugins/アプリ間レコード同期/specification.md` を確認する。
2. `plugins/アプリ間レコード同期/codex/troubleshooting.md`、`plugins/アプリ間レコード同期/codex/handover-YYYY-MM-DD.md`、`plugins/アプリ間レコード同期/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/アプリ間レコード同期/CrossAppRecordSync` であること。
- 難読化済みファイル `plugins/アプリ間レコード同期/CrossAppRecordSynce` を直接編集していないこと。
- `plugins/アプリ間レコード同期/CrossAppRecordSync/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/アプリ間レコード同期/CrossAppRecordSync` から実行します。
- 難読化済みファイルは `plugins/アプリ間レコード同期/CrossAppRecordSynce` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/アプリ間レコード同期/CrossAppRecordSync` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/アプリ間レコード同期/CrossAppRecordSynce` の `contents` 配下を基準に作成します。
...
```text
plugins/アプリ間レコード同期/codex/handover-YYYY-MM-DD.md
plugins/アプリ間レコード同期/codex/next-tasks.md
```
...
```text
CrossAppRecordSync-v<version>.zip
```

難読化版ZIP:

```text
CrossAppRecordSync-ev<version>.zip
```
