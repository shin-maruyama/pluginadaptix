...
(async function ($, PLUGIN_ID) {
  ...
  obj.events1.type = 'app.record.index.show';
  ...
  obj.events1.synchronize = async function () {
    ...
    const filterList = await obj.func.filterRecordOrRowOfTable(sourceAllRecords);
    //filterList.reverse();
  ...
  obj.events1.checkFields = async function () {
    const destinationFields = await getFieldList(kintone.app.getId());
  ...
  obj.events2.type = ['app.record.create.show', 'app.record.edit.show'];
  ...
  async function outputCSV(records) {
    ...
    records.reverse();
    const fields = await getFieldList(kintone.app.getId());
