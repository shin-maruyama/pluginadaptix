...
1. 履歴書き換え前の既存クローン利用者へ再クローンを依頼する。
...
秘密鍵については、2026-07-16にローカルファイルを削除し、`git filter-repo` でローカルおよびGitHubリモートの全5ブランチから除去した。タグは存在せず、GitHub管理のPR参照は秘密鍵追加前のコミットであることを確認した。
