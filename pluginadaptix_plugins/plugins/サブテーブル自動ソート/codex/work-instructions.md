...
```text
plugins/サブテーブル自動ソート/SubTableAutosort
```

## 難読化版場所

```text
plugins/サブテーブル自動ソート/SubTableAutosorte
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/サブテーブル自動ソート/SubTableAutosort`
- 難読化版: `plugins/サブテーブル自動ソート/SubTableAutosorte`
...
1. `plugins/サブテーブル自動ソート/specification.md` を確認する。
2. `plugins/サブテーブル自動ソート/codex/troubleshooting.md`、`plugins/サブテーブル自動ソート/codex/handover-YYYY-MM-DD.md`、`plugins/サブテーブル自動ソート/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/サブテーブル自動ソート/SubTableAutosort` であること。
- 難読化済みファイル `plugins/サブテーブル自動ソート/SubTableAutosorte` を直接編集していないこと。
- `plugins/サブテーブル自動ソート/SubTableAutosort/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/サブテーブル自動ソート/SubTableAutosort` から実行します。
- 難読化済みファイルは `plugins/サブテーブル自動ソート/SubTableAutosorte` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/サブテーブル自動ソート/SubTableAutosort` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/サブテーブル自動ソート/SubTableAutosorte` の `contents` 配下を基準に作成します。
...
```text
plugins/サブテーブル自動ソート/codex/handover-YYYY-MM-DD.md
plugins/サブテーブル自動ソート/codex/next-tasks.md
```
