# FieldHidden バグ修正準備計画

作業日: 2026-06-26

## 1. 修正対象バグ

- BUG-011: No.1 2026-24-06 BUG-011 DOM取得失敗時にnullへhasAttributeを呼ぶ
- BUG-012: No.2 2026-24-06 BUG-012 関連属性の該当設定がない場合にfields参照で落ちる
- BUG-022: No.3 2026-24-06 BUG-022 設定未保存/破損時にJSON.parseでロード失敗する

## 2. 優先順位

1. BUG-012: 重大度 High、確度 高確度
2. BUG-011: 重大度 High、確度 高確度
3. BUG-022: 重大度 Medium、確度 高確度
...
- BUG-011: relatedFieldsGet（41行付近）
- BUG-012: relatedFieldsGet（41行付近）
- BUG-022: 無名イベントハンドラまたはトップレベル処理

## 5. 修正方針

- BUG-011: `if (!fieldWrap2) return;` を追加し、関連属性の存在確認を行う。
- BUG-012: `getAttributeTarget` と `fields` の存在確認を追加する。
- BUG-022: 設定存在チェックとJSON parse失敗時の安全なreturnを追加する。mobile.jsも同様確認。

## 6. 影響範囲

- BUG-011: 高 - 対象フィールドのDOMが存在しない、折りたたみ、削除、他プラグイン非表示などの条件で実行時エラーになる。
- BUG-012: 高 - ドロップダウン/ラジオ連動属性の選択肢名が一致しない場合に画面表示処理が停止する。
- BUG-022: 中 - 設定未保存、設定破損、旧設定形式の場合にプラグイン全体がロード時点で停止する。
