...
(async function ($, PLUGIN_ID) {
  ...
  if (!(await KNTP162910certification())) {
    return;
  }

  const $submitButton = $('.kintoneplugin-button-dialog-ok');
  const $cancelButton = $('.js-cancel-button');
  const config = kintone.plugin.app.getConfig(PLUGIN_ID);

  let fieldList = await getFieldList();
  fieldList = filterField(fieldList, false, 'REFERENCE_TABLE', 'CALC', 'GROUP', 'HR', 'LABEL', 'SPACER');
  ...
  $submitButton.on('click', function () {
    ...
    const config = { elementArray: JSON.stringify(elementArray), color: color };
    // console.log(config.elementArray);
