  ...
  limit.eventCreate = function () {
    ...
    for (let i = 0; i < config.field.length; i++) {
      if (config.field[i] && config.field[i] !== '') {
        events.push('app.record.edit.change.' + config.field[i].split('　')[1]);
        events.push('app.record.create.change.' + config.field[i].split('　')[1]);
        events.push('app.record.index.edit.change.' + config.field[i].split('　')[1]);
      }
    }
    return events;
  };
