...
- 難読化版: `plugins/条件付き経過時間自動計算/ConditionalElapsedTimeAutomaticCalculatione`
...
1. `plugins/条件付き経過時間自動計算/specification.md` を確認する。
2. `plugins/条件付き経過時間自動計算/codex/troubleshooting.md`、`plugins/条件付き経過時間自動計算/codex/handover-YYYY-MM-DD.md`、`plugins/条件付き経過時間自動計算/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/条件付き経過時間自動計算/ConditionalElapsedTimeAutomaticCalculation` であること。
- 難読化済みファイル `plugins/条件付き経過時間自動計算/ConditionalElapsedTimeAutomaticCalculatione` を直接編集していないこと。
- `plugins/条件付き経過時間自動計算/ConditionalElapsedTimeAutomaticCalculation/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/条件付き経過時間自動計算/ConditionalElapsedTimeAutomaticCalculation` から実行します。
- 難読化済みファイルは `plugins/条件付き経過時間自動計算/ConditionalElapsedTimeAutomaticCalculatione` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/条件付き経過時間自動計算/ConditionalElapsedTimeAutomaticCalculation` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/条件付き経過時間自動計算/ConditionalElapsedTimeAutomaticCalculatione` の `contents` 配下を基準に作成します。
...
```text
plugins/条件付き経過時間自動計算/codex/handover-YYYY-MM-DD.md
plugins/条件付き経過時間自動計算/codex/next-tasks.md
```
...
```text
plugins/条件付き経過時間自動計算/decisions/
plugins/条件付き経過時間自動計算/codex/troubleshooting.md
```
