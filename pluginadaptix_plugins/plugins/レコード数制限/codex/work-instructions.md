...
```text
plugins/レコード数制限/RecordLimit
```

## 難読化版場所

```text
plugins/レコード数制限/RecordLimite
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/レコード数制限/RecordLimit`
- 難読化版: `plugins/レコード数制限/RecordLimite`
...
1. `plugins/レコード数制限/specification.md` を確認する。
2. `plugins/レコード数制限/codex/troubleshooting.md`、`plugins/レコード数制限/codex/handover-YYYY-MM-DD.md`、`plugins/レコード数制限/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/レコード数制限/RecordLimit` であること。
- 難読化済みファイル `plugins/レコード数制限/RecordLimite` を直接編集していないこと。
- `plugins/レコード数制限/RecordLimit/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/レコード数制限/RecordLimit` から実行します。
- 難読化済みファイルは `plugins/レコード数制限/RecordLimite` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/レコード数制限/RecordLimit` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/レコード数制限/RecordLimite` の `contents` 配下を基準に作成します。
...
```text
plugins/レコード数制限/codex/handover-YYYY-MM-DD.md
plugins/レコード数制限/codex/next-tasks.md
```
