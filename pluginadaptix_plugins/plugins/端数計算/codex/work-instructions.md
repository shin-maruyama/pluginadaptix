...
```text
plugins/端数計算/FractionCalculation
```

## 難読化版場所

```text
plugins/端数計算/FractionCalculatione
```

## 元ソースと難読化版の対応関係

- 元ソース: `plugins/端数計算/FractionCalculation`
- 難読化版: `plugins/端数計算/FractionCalculatione`
...
1. `plugins/端数計算/specification.md` を確認する。
2. `plugins/端数計算/codex/troubleshooting.md`、`plugins/端数計算/codex/handover-YYYY-MM-DD.md`、`plugins/端数計算/codex/next-tasks.md` を確認する。
...
- 修正対象が元ソース `plugins/端数計算/FractionCalculation` であること。
- 難読化済みファイル `plugins/端数計算/FractionCalculatione` を直接編集していないこと。
- `plugins/端数計算/FractionCalculation/contents/manifest.json` が存在すること。
...
- 要確認
- 難読化は元ソース `plugins/端数計算/FractionCalculation` から実行します。
- 難読化済みファイルは `plugins/端数計算/FractionCalculatione` に生成されるものとして扱います。
- 難読化済みファイルは直接修正しません。
...
- 通常版ZIPは元ソース `plugins/端数計算/FractionCalculation` の `contents` 配下を基準に作成します。
- 難読化版ZIPは難読化済み版 `plugins/端数計算/FractionCalculatione` の `contents` 配下を基準に作成します。
...
```text
plugins/端数計算/codex/handover-YYYY-MM-DD.md
plugins/端数計算/codex/next-tasks.md
```
